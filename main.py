...
def get_args_parser():
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='./pretrain/eformer_s2_450.pth',
                        help='finetune from checkpoint')

    # Dataset parameters
    parser.add_argument('--datalist-path', default='./datalist/train_v1.txt', type=str,
                        help='dataset list path')
    parser.add_argument('--data-path', default='', type=str,
                        help='dataset path')
    parser.add_argument('--score_out_path', default='score_out.txt', type=str,
                        help='score output path')
    parser.add_argument('--has_label', default=False, help='score out with label')

    parser.add_argument('--data-set', default='SUHIFI', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'SUHIFI'],
    ...
    # rdrop settings
    parser.add_argument('--rdrop_bsize', default=16, type=int,
                        help='number of samples per batch for r-drop')
    parser.add_argument('--max_rdrop_epoch', default=10, type=int,
                        help='first n epoch to activate r-drop')
...
def main(args):
    ...
    if args.finetune:
        ...
        for k, v in state_dict.items():
            kc = k
            if 'patch_embed_hp' in k:
                kc = k.replace('patch_embed_hp','patch_embed')
            if 'network_hp' in k:
                kc = k.replace('network_hp','network')
            if 'norm_hp' in k:
                kc = k.replace('norm_hp','norm')

            if kc in checkpoint_model and checkpoint_model[kc].shape == state_dict[k].shape:
                #print(k)
                state_dict[k] = checkpoint_model[kc]
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, args.score_out_path, args.has_label)
        print("Test finished.")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_grad, args.clip_mode, args.rdrop_bsize, args.max_rdrop_epoch, model_ema, mixup_fn,
        )

        lr_scheduler.step(epoch)
        if args.output_dir:
            checkpoint_paths = [output_dir / 'best.pth']
            #checkpoint_paths = [str(output_dir)+'/ep'+str(epoch)+'.pth']
