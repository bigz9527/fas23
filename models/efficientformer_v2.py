...
class EfficientFormerV2(nn.Module):
    def __init__(self, layers, embed_dims=None,
        ...
        self.patch_embed_hp = stem(3, embed_dims[0], act_layer=act_layer)
        ...
        self.network_hp = copy.deepcopy(self.network)
        ...
        if self.fork_feat:
            ...
            self.norm_hp = norm_layer(embed_dims[-1])
            self.head = nn.Linear(
                2 * embed_dims[-1], num_classes) if num_classes > 0 \
                else nn.Identity()
    ...
    def forward_tokens_hp(self, x):
        outs = []
        for idx, block in enumerate(self.network_hp):
            x = block(x)
            if self.fork_feat and idx in self.out_indices:
                norm_layer = getattr(self, f'norm{idx}')
                x_out = norm_layer(x)
    ...
    def forward(self, x, x_hp):

        x = self.patch_embed(x)
        x = self.forward_tokens(x)

        x_hp = self.patch_embed_hp(x_hp)
        x_hp = self.forward_tokens_hp(x_hp)
        ...
        # print(x.size())
        x = self.norm(x)
        x_hp = self.norm_hp(x_hp)

        x = torch.cat((x, x_hp), 1)
